...
public class Interaction : MonoBehaviour
{
    ...
    {
        if (Physics.Raycast(playerCamera.transform.position, playerCamera.transform.forward, out padlock, 2f))
        {
            if (padlock.collider.gameObject.layer == 9)
            {
                return true;
            }
            else
